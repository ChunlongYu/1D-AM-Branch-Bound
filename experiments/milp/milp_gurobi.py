...
def main():
    ...
    print(f"=== MILP  instance={name}  n={len(parts)}  M={M}  sym_break={sym}  TL={tl}s ===")
    ...
    print(f"RESULT instance={name} n={len(parts)} M={M} obj={r['obj']:.4f} "
          f"bound={r['bound']:.4f} gap={r['gap']:.6f} status={r['status']} time={r['time']:.2f}")
